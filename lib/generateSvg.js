...
function compPerson(rng, b, paths) {
  // 人-like: a compact roof and descending sweep.
  const apexX = b.x + b.w * (0.48 + rng() * 0.04);
  const apexY = b.y + b.h * 0.14;
  add(paths, strokeSweep(rng, apexX, apexY, b.x + b.w * 0.22, b.y + b.h * 0.86, -1));
  add(paths, strokeSweep(rng, apexX, apexY, b.x + b.w * 0.78, b.y + b.h * 0.90, +1));
}

function compSmall(rng, b, paths) {
  // 小-like: a central vertical with balanced side dots.
  const cx = b.x + b.w * 0.50;
  add(paths, strokeLine(cx, b.y + b.h * 0.18, cx, b.y + b.h * 0.88));
  add(paths, strokeDot(rng, b.x + b.w * 0.28, b.y + b.h * 0.43, b.w * 0.10));
  add(paths, strokeDot(rng, b.x + b.w * 0.72, b.y + b.h * 0.43, b.w * 0.10));
}

function compRoof(rng, b, paths) {
  // 宀-like: roof with a compact interior stroke.
  const cx = b.x + b.w * 0.50;
  const yTop = b.y + b.h * 0.16;
  const yRoof = b.y + b.h * 0.42;
  add(paths, strokeDot(rng, cx, yTop, b.w * 0.07));
  add(paths, `${moveTo(b.x + b.w * 0.18, yRoof)} ${lineTo(cx, b.y + b.h * 0.28)} ${lineTo(b.x + b.w * 0.84, yRoof)}`);
  add(paths, strokeLine(b.x + b.w * 0.30, b.y + b.h * 0.62, b.x + b.w * 0.72, b.y + b.h * 0.62));
  if (chance(rng, 0.55)) {
    add(paths, strokeHookDown(rng, cx, b.y + b.h * 0.55, b.y + b.h * 0.86, 1, b.w * 0.09));
  }
}

function compThread(rng, b, paths) {
  // 糸-like: stacked small strokes and a central tie.
  const cx = b.x + b.w * 0.50;
  add(paths, strokeDot(rng, b.x + b.w * 0.34, b.y + b.h * 0.22, b.w * 0.07));
  add(paths, strokeDot(rng, b.x + b.w * 0.66, b.y + b.h * 0.22, b.w * 0.07));
  add(paths, strokeLine(b.x + b.w * 0.26, b.y + b.h * 0.50, b.x + b.w * 0.74, b.y + b.h * 0.50));
  add(paths, strokeHookDown(rng, cx, b.y + b.h * 0.38, b.y + b.h * 0.84, -1, b.w * 0.09));
}

function compFire(rng, b, paths) {
  // 火-like: central stem with four controlled sweeps.
  const cx = b.x + b.w * 0.50;
  const cy = b.y + b.h * 0.48;
  add(paths, strokeLine(cx, b.y + b.h * 0.14, cx, b.y + b.h * 0.88));
  add(paths, strokeSweep(rng, cx, cy, b.x + b.w * 0.23, b.y + b.h * 0.34, -1));
  add(paths, strokeSweep(rng, cx, cy, b.x + b.w * 0.78, b.y + b.h * 0.30, +1));
  add(paths, strokeSweep(rng, cx, cy, b.x + b.w * 0.25, b.y + b.h * 0.86, -1));
  add(paths, strokeSweep(rng, cx, cy, b.x + b.w * 0.78, b.y + b.h * 0.86, +1));
}

// --- Legacy decorative motifs (not selected by the CJK generator) ---
...
function radicalHeartLeft(rng, b, paths) {
  // 忄-ish: vertical plus two carefully spaced dots.
  const x = b.x + b.w * 0.54;
  add(paths, strokeLine(x, b.y + b.h * 0.16, x, b.y + b.h * 0.88));
  add(paths, strokeDot(rng, b.x + b.w * 0.28, b.y + b.h * 0.30, b.w * 0.08));
  add(paths, strokeDot(rng, b.x + b.w * 0.28, b.y + b.h * 0.66, b.w * 0.09));
}

function radicalSpeechLeft(rng, b, paths) {
  // 讠-ish: short top stroke and descending hook.
  add(paths, strokeLine(b.x + b.w * 0.20, b.y + b.h * 0.24, b.x + b.w * 0.78, b.y + b.h * 0.24));
  add(paths, strokeHookDown(rng, b.x + b.w * 0.56, b.y + b.h * 0.38, b.y + b.h * 0.88, -1, b.w * 0.10));
}
...
function compRandomStrokes(rng, b, paths, targetCount) {
  // Add only restrained, glyph-like detail; never decorative waves or circles.
  const gridN = 7;
  const j = Math.min(b.w, b.h) * 0.012;
  let attempts = 0;

  while (paths.length < targetCount && attempts++ < targetCount * 10) {
    const kind = pick(rng, ['h', 'h', 'v', 'v', 'd1', 'd2', 'hook', 'dot']);
...
function pickMainComponent(rng) {
  return pick(rng, [
    // CJK-inspired bases only. Their shared stroke grammar keeps every layout coherent.
    compSun,
    compField,
    compMouth,
    compTree,
...
function generateGlyphPaths(hex64, pad) {
  ...
  // Add restrained detail only when a composition is genuinely sparse.
  const target = paths.length + intIn(rng, 2, 6);
  compRandomStrokes(rng, bounds, paths, target);

  // A short stem can tie otherwise sparse compositions together.
  if (paths.length < 10 && chance(rng, 0.28)) {
    const cx = bounds.x + bounds.w * 0.50;
    add(paths, strokeLine(cx, bounds.y + bounds.h * 0.28, cx, bounds.y + bounds.h * 0.76));
  }
...
function generateSvg(hex64, opts) {
  ...
  // Square caps and mitered joins keep the result closer to a clean glyph
  // than an illustration, while vector-effect preserves the requested weight.
  return (
    `<?xml version="1.0" encoding="UTF-8"?>` +
    `<svg xmlns="http://www.w3.org/2000/svg" width="${escapeAttr(size)}" height="${escapeAttr(size)}" viewBox="0 0 ${VB} ${VB}" fill="none">` +
    bgRect +
    `<g stroke="${escapeAttr(fg)}" stroke-width="${escapeAttr(stroke)}" stroke-linecap="square" stroke-linejoin="miter" stroke-miterlimit="2" vector-effect="non-scaling-stroke">` +
    pathEls +
    `</g>` +
    `</svg>`
  );
